...
import { deletehighlight } from '@/api/react-query/mutations/highlightMutations';
...
export function DataTableRowActions<TData>({
  ...
  const handleConfirm = async () => {
    try {
      const res = await deletehighlight(String(deleteId));
      // console.log(res);
      if (res.message) {
        toast.warning('highlight deleted successfully.');
        router.refresh();
      } else {
      ...
      toast.error('Cannot delete highlight.');
    }
  };
  ...
  const handelEditClick = (row: any) => {
    ...
    if (Number(id) > 0) {
      router.push(
        `/dashboard/settings/highlight-spotlight/edit/${row?.original?.id}`
      );
    }
  };
